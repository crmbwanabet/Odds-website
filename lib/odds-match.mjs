...
// Ensure competitor odds never exceed BwanaBet odds on any outcome (home, draw, away).
// If a competitor's price is higher than or equal to BwanaBet's, cap it at (bwana - 0.03).
export function capOddsAgainstBwana(bwanaOdds, compOdds) {
    if (!compOdds) return null;
    if (!bwanaOdds) return compOdds;
    const out = { ...compOdds };
    for (const k of ['home', 'draw', 'away']) {
        const b = parseFloat(bwanaOdds[k]);
        const c = parseFloat(compOdds[k]);
        if (!isNaN(b) && !isNaN(c)) {
            const capped = Math.min(c, b - 0.03);
            out[k] = Math.max(0.01, capped).toFixed(2);
        }
    }
    return out;
}
...
export function matchCompetitors(homeTeam, awayTeam, index, keys, bwanaOdds) {
    ...
    for (const key of keys) {
        ...
        if (best) {
            const capped = bwanaOdds ? capOddsAgainstBwana(bwanaOdds, best.odds) : best.odds;
            rows.push({ key, odds: capped });
        }
    }
    return rows;
}
